...
public class App {
    ...
    // 5. VULNERABLE: Generate Invoice (SQL Injection Risk)
    public String generateInvoice(int orderId, double totalAmount) {
        // Simulated SQL Injection vulnerability
        String query = "SELECT * FROM Invoices WHERE orderId = " + orderId + " AND amount = " + totalAmount;
    ...
    // 8. VULNERABLE: Apply Promo Code (Insufficient Authorization)
    public double applyPromoCode(double totalAmount, String promoCode) {
        // No checks for valid users or promo code rules
    ...
    // 11. VULNERABLE: Log Sensitive Information
    public void logSensitiveInfo(String username, String password) {
        // Logging credentials directly (Security flaw: sensitive data exposure)
